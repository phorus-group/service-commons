...
Binaries and dependency information for Maven and Gradle can be found at [http://search.maven.org](https://search.maven.org/search?q=g:group.phorus%20AND%20a:service-commons).

<details open>
<summary>Gradle / Kotlin DSL</summary>

```kotlin
implementation("group.phorus:service-commons:x.y.z")
```
</details>
...
- [exception-handling](https://github.com/phorus-group/exception-handling), standardized error responses (`NotFound`, `BadRequest`, etc.), and automatic exception handling

Both are included automatically when you add `service-commons` to your project. You can use their
...
- **Generic CRUD service**: provides `findById`, `create`, `update`, `replace`, and `delete` out of the box. You define your entity, DTO, and response types, and the library handles the rest.
- **Generic CRUD controller**: exposes `GET /{id}`, `POST`, `PATCH /{id}`, `PUT /{id}`, `DELETE /{id}` REST endpoints. A single class with zero boilerplate gives you a fully functional API.
...
- **Error handling** via [exception-handling](https://github.com/phorus-group/exception-handling): when an entity is not found, the library throws `NotFound`, which is automatically converted to a structured `404` JSON response.
...
[exception-handling](https://github.com/phorus-group/exception-handling).
...
> This section covers general JPA/Hibernate optimization techniques. It is not specific to
> service-commons, but is useful when working with lazy-loaded relationships.
